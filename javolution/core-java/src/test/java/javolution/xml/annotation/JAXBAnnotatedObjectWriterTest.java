...
public class JAXBAnnotatedObjectWriterTest {
	...
	@Test
	public void testSmall() throws JAXBException{
		final JAXBAnnotatedObjectWriter jaxbWriter = new JAXBAnnotatedObjectWriterImpl(TestRoot.class);
		...
		final TestElement element1 = new TestElement();
		element1.setTestIntElement(1);
		element1.setTestBooleanElement(true);

		final TestElement element2 = new TestElement();
		element2.setTestDoubleElement(1.5);
		element2.setTestFloatElement(1.0f);

		testRoot.getTestElement().add(element1);
		testRoot.getTestElement().add(element2);
		...
		assertEquals("Case 1 XML", EXPECTED_SMALL, writer.toString());
	}
